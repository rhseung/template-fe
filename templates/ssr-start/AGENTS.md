...
`csr`과 라우터가 같다(둘 다 TanStack Router). 달라지는 건 `main.tsx`/`index.html`이
없고 `router.tsx`가 `getRouter()`를 export하며(TanStack Start가 이 이름으로 찾는다),
`routes/__root.tsx`가 `component` 대신 `shellComponent`로 `<html>` 전체를 그린다는 것뿐이다.
`src/server.ts`는 커스텀 서버 로직이 필요해지기 전까진 만들지 않는다 — 없으면 TanStack
Start가 기본 핸들러를 자동으로 쓴다.
...
### `RootDocument`가 마운트 전엔 아무것도 안 그리나

`routes/__root.tsx`의 `RootDocument`는 `useState(false)`로 시작해서 `useEffect`가
붙기 전엔 children을 안 그린다. csr/ssg/ssr-next와 같은 이유 두 가지가 SSR에서는
**실제로 하이드레이션을 깨는 문제**가 된다(CSR/정적 아일랜드에선 취향 문제였지만, 여긴
서버가 진짜로 HTML을 미리 그려서 클라이언트가 그 위에 하이드레이션한다):

1. **언어 감지가 서버·클라이언트에서 다르다.** `detectLanguage()`는 `navigator.language`를
   보는데, 서버 렌더 시점엔 `navigator`가 없어 항상 폴백(`ko`)이다. 클라이언트가 실제
   브라우저 언어로 바로 그려버리면 서버가 그린 HTML과 문구가 달라 hydration mismatch가 난다.
2. **MSW가 서비스 워커라 비동기로 뜬다.** 워커가 뜨기 전에 라우트 트리가 먼저 그려지면
   실제(목킹 안 된) fetch가 나갈 수 있다.

서버 렌더와 클라이언트의 첫 렌더(하이드레이션 이전)가 둘 다 "아직 없음"으로 일치해야
하고, `useEffect`가 마운트 후에야 목을 켜고 진짜 화면을 그린다. **트레이드오프**: 자바
스크립트가 실행되기 전까지 사용자는 빈 화면을 본다 — TanStack Start의 SSR이 주는 "즉시
콘텐츠"라는 이점을 이 예제 화면은 못 누린다. 실제 백엔드를 붙여 MSW를 끄고 라우트
`loader`로 서버에서 데이터를 미리 채우면(그리고 언어를 쿠키/`Accept-Language`로 서버에서
정하면) 이 게이팅을 걷어내고 진짜 SSR 콘텐츠를 그릴 수 있다.

next-themes가 하이드레이션 전에 `<html>`의 class를 스크립트로 바꾸는 것도 마찬가지로
서버·클라이언트가 항상 다르다 — 이건 라이브러리가 의도한 동작이라 `<html
suppressHydrationWarning>`로 경고만 죽인다.
...
- 진짜 SSR 데이터 페칭(로더에서 미리 채우기)이 필요해지면 `Route`의 `loader`에서
  `queryClient.ensureQueryData(getXxxOptions())`를 부른다 — 지금 예제는 안 쓴다
  (위 "`RootDocument`가 마운트 전엔…" 절 참고).
...
- `navigator.language`로 브라우저 언어를 감지하는데, 서버 렌더 시점엔 `navigator`가 없다 —
  "`RootDocument`가 마운트 전엔…" 절 참고. `?.language` 한 군데만 옵셔널 체이닝하면 다음
...
- **Storybook은 `@storybook/react-vite`를 그대로 쓴다**, `@storybook/tanstack-react`가
  아니다 — 후자는 `definePreview`/route-typed `Meta`라는 다른 CSF 패러다임이라 스토리
  전체를 새로 써야 한다. 대신 `.storybook/decorators.tsx`의 손으로 쓴 `withRouter`
  데코레이터(csr과 동일, 12줄)가 `<Link>`/`useNavigate()` 스코프를 채워준다.
...
## 12. 배포 — Vercel은 nitro가 베타다

Cloudflare Workers는 `@cloudflare/vite-plugin` + `wrangler`로 안정적이다(csr/ssg/ssr-next와
같은 성숙도). **Vercel 배포는 `nitro`(`nitro/vite` 플러그인) 경유인데, 2026-08 기준 `nitro`
패키지의 `latest` 태그가 여전히 `3.0.260610-beta`다** — 정식 릴리스가 아니다. 나머지 세
템플릿보다 프로덕션 신뢰도가 낮다는 뜻이니, Vercel에 실제로 배포하기 전에
[nitro 릴리스](https://github.com/nitrojs/nitro/releases)를 확인해라. 이게 이 템플릿이
계획에서 의도적으로 마지막 순서였던 이유다.

### cloudflare 타깃에서만 `dev`/`preview`가 Bun이 아니라 Node로 돈다

`@cloudflare/vite-plugin`은 로컬 workerd로 가는 웹소켓 브리지를 붙이는데, Bun의 `ws`
구현이 그 브리지가 쓰는 `'upgrade'` 이벤트를 아직 지원하지 않는다 — `bun run dev`를
Bun 런타임으로 그대로 돌리면 포트를 절대 못 열고 조용히 멈춘다(직접 재현해서 확인함,
Node로 돌리면 정상 동작). 그래서 `bun run init --target=cloudflare`가 `dev`/`preview`
스크립트를 `node node_modules/vite/bin/vite.js dev …`로 덮어쓴다 — `bun run dev`라는
명령 자체는 그대로지만, 실제로 vite를 실행하는 건 Node다. `vercel`/`none` 타깃은 이
문제가 없다(재현해서 확인함) — Bun 그대로 돈다. `e2e/`의 Playwright `webServer`도 타깃에
무관하게 항상 Node로 고정해뒀다(`playwright.config.ts`).
